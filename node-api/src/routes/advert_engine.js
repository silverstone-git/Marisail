  ...
  try {
    ...
    return res
      ...
      .json({ ok: true,
        ...
        engine_controlsystem: engineEquipmentFields.map((row) => row.engine_controlsystem),
        ...
        protection_covers: engineEquipmentFields.map((row) => row.protection_covers),
        crankcase_ventilation: engineEquipmentFields.map((row) => row.crankcase_ventilation),
        heat_exchanger: engineEquipmentFields.map((row) => row.heat_exchanger),
        heat_exchanger_WET: engineEquipmentFields.map((row) => row.heat_exchanger_WET),
        seawater_pump: engineEquipmentFields.map((row) => row.seawater_pump),
        charge_aircooler: engineEquipmentFields.map((row) => row.charge_aircooler),
        working_principle: engineEquipmentFields.map((row) => row.working_principle),
        compression_ratio: engineEquipmentFields.map((row) => row.compression_ratio),
        pistonspeed_1500: engineEquipmentFields.map((row) => row.pistonspeed_1500),
        pistonspeed_1800: engineEquipmentFields.map((row) => row.pistonspeed_1800),
        ...
        auxiliarypower_takeoff: engineEquipmentFields.map((row) => row.auxiliarypower_takeoff),
        remote_controlsystems: engineEquipmentFields.map((row) => row.remote_controlsystems),

        oil_filter: engineOilFields.map((row) => row.oil_filter),
        oil_filtertype: engineOilFields.map((row) => row.oil_filtertype),
        centrifugal_oilcleaner: engineOilFields.map((row) => row.centrifugal_oilcleaner),
        ...
        lubrication_oilpressure: engineOilFields.map((row) => row.lubrication_oilpressure),
        oilfilter_bypassvalve: engineOilFields.map((row) => row.oilfilter_bypassvalve),
        ...
        lowest_fuelconsumption: engineFuelFields.map((row) => row.lowest_fuelconsumption),
        fuel_consumptionrate: engineFuelFields.map((row) => row.fuel_consumptionrate),
        FC_fullload: engineFuelFields.map((row) => row.FC_fullload),
        FuelInjection_systemtype: engineFuelFields.map((row) => row.FuelInjection_systemtype),
        Fuel_deliverypressure: engineFuelFields.map((row) => row.Fuel_deliverypressure),
        ...
        enginemounting_orientation: engineMountingFields.map((row) => row.enginemounting_orientation),
        engine_suspension: engineMountingFields.map((row) => row.engine_suspension),
        engine_mountingtype: engineMountingFields.map((row) => row.engine_mountingtype),
        mountingbracket_material: engineMountingFields.map((row) => row.mountingbracket_material),
        alignment_requirements: engineMountingFields.map((row) => row.alignment_requirements),
        engine_block: engineMountingFields.map((row) => row.engine_block),

        engine_monitoringsystem: engineSafetyFields.map((row) => row.engine_monitoringsystem),
        overheat_protection: engineSafetyFields.map((row) => row.overheat_protection),
        lowoil_pressurealarm: engineSafetyFields.map((row) => row.lowoil_pressurealarm),
        emergency_stopsystem: engineSafetyFields.map((row) => row.emergency_stopsystem),

        propulsion: enginePropulsionFields.map((row) => row.propulsion),
        propeller_bladematerial: enginePropulsionFields.map((row) => row.propeller_bladematerial),
        propeller_shaftmaterial: enginePropulsionFields.map((row) => row.propeller_shaftmaterial),
        steering_system: enginePropulsionFields.map((row) => row.steering_system),
        steering_controltype: enginePropulsionFields.map((row) => row.steering_controltype),
        trim_system: enginePropulsionFields.map((row) => row.trim_system),
        trim_tabmaterial: enginePropulsionFields.map((row) => row.trim_tabmaterial),
        trim_tab_type: enginePropulsionFields.map((row) => row.trim_tab_type),
        bowthruster: enginePropulsionFields.map((row) => row.bowthruster),
        propulsion_system: enginePropulsionFields.map((row) => row.propulsion_system),
        propulsion_systemtype: enginePropulsionFields.map((row) => row.propulsion_systemtype),
        propeller_diameter: enginePropulsionFields.map((row) => row.propeller_diameter),
        propeller_material: enginePropulsionFields.map((row) => row.propeller_material),
        ...
        propeller_shaftdiameter: enginePropulsionFields.map((row) => row.propeller_shaftdiameter),
        gearbox_type: enginePropulsionFields.map((row) => row.gearbox_type),
        transmission_cooling: enginePropulsionFields.map((row) => row.transmission_cooling),

        transmission_type: engineTransmissionFields.map((row) => row.transmission_type),
        ...
        flywheel_housing: engineTransmissionFields.map((row) => row.flywheel_housing),
        camshaft: engineTransmissionFields.map((row) => row.camshaft),
        crankshaft_alloy: engineTransmissionFields.map((row) => row.crankshaft_alloy),
        crankcase_design: engineTransmissionFields.map((row) => row.crankcase_design),

        after_cooled: engineCoolingFields.map((row) => row.after_cooled),
        cooling_system: engineCoolingFields.map((row) => row.cooling_system),
        closed_coolingsystem: engineCoolingFields.map((row) => row.closed_coolingsystem),
        open_coolingsystem: engineCoolingFields.map((row) => row.open_coolingsystem),
        intercooled: engineCoolingFields.map((row) => row.intercooled),
        recommended_coolant: engineCoolingFields.map((row) => row.recommended_coolant),
        cooling_type: engineCoolingFields.map((row) => row.cooling_type),
        heat_exchangermaterial: engineCoolingFields.map((row) => row.heat_exchangermaterial),
        heat_dissipationrate: engineCoolingFields.map((row) => row.heat_dissipationrate),
        engine_lubrication: engineCoolingFields.map((row) => row.engine_lubrication),
        lubrication_system: engineCoolingFields.map((row) => row.lubrication_system),
        cooling_fluidtype: engineCoolingFields.map((row) => row.cooling_fluidtype),
        cooling_systempressure: engineCoolingFields.map((row) => row.cooling_systempressure),
        air_filtertype: engineCoolingFields.map((row) => row.air_filtertype),
        circulation_pumptype: engineCoolingFields.map((row) => row.circulation_pumptype),
        rawwater_pumptype: engineCoolingFields.map((row) => row.rawwater_pumptype),

        alternator: engineElectricalFields.map((row) => row.alternator),
        alternator_output: engineElectricalFields.map((row) => row.alternator_output),
        ...
        alternator_outputAMPS: engineElectricalFields.map((row) => row.alternator_outputAMPS),
        starter_MotorVoltage: engineElectricalFields.map((row) => row.starter_MotorVoltage),
        ECU_Model: engineElectricalFields.map((row) => row.ECU_Model),
        Battery_ChargingSystem: engineElectricalFields.map((row) => row.Battery_ChargingSystem),
        integrated_generator: engineElectricalFields.map((row) => row.integrated_generator),
        battery_voltagenumber: engineElectricalFields.map((row) => row.battery_voltagenumber),
